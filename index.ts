...
interface SlackEventRequest extends SlackRequest {
  event: SlackEvent,
}

interface SlackVerifyRequest extends SlackRequest {
  challenge: string,
}
...
const challenge = async (data: SlackVerifyRequest, resp: cloud.Response): Promise<void> => {
  console.log("Running challenge")
  resp.status(200).json({ challenge: data.challenge })
}

const processMessage = async (event: SlackEvent, resp: cloud.Response): Promise<void> => {
...
const handler = async (data: SlackRequest, resp: cloud.Response): Promise<void> => {
  console.log("Choosing event handler")
  switch (data.type) {
    case "url_verification":
      await challenge((<SlackVerifyRequest>data), resp)
      break

    case "event_callback":
      await processMessage((<SlackEventRequest>data).event, resp)
      break
...
endPoint.post("/cake/events", async (req, resp) => {
  ...
  let data: SlackVerifyRequest | SlackEventRequest = JSON.parse(req.body.toString())
